...
# GET /looks/search API 3.0
    def search_looks(self, user_id, fields=''):
        url = '{}{}{}'.format(self.host,'looks/search?user_id=',user_id)
        print url
        params = {"fields":fields}
        r = self.session.get(url,params=params)
        if r.status_code == requests.codes.ok:
...
# GET /users
    def get_all_users(self,looker_user_ids):
        url = '{}{}/search?id={}'.format(self.host,'users',looker_user_ids)
        print(url)
        params = {}
        r = self.session.get(url,params=params)
        if r.status_code == requests.codes.ok:
